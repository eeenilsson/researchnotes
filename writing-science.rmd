...
> Describe the results in words and make references to tables and figures. Point out interesting results, especially those you bring up in the discussion section. Make connections and summarise patterns. All tables and figures should be referenced from the text (**Table \@ref(tab:table-one-example)**).
...
It is common to include a "Table 1" with descriptive statistics on the study group(s). It is usually clearer to separate categorical variables (eg percentages, frequencies) from numerical ones into separate tables, but since there is a limit on the number of tables in the paper one usually has to either combine them in the same table or (if there are not too many variables of a certain kind) to report some of it in the text instead of a table.
